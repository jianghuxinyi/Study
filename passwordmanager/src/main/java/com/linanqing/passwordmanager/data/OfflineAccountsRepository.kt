...
class OfflineAccountsRepository(private val accountDao: AccountDao) : AccountsRepository {
    ...
    override suspend fun insertAccount(item: Account) = accountDao.insert(item)

    override suspend fun deleteAccount(item: Account) = accountDao.delete(item)

    override suspend fun updateAccount(item: Account) = accountDao.update(item)
}
